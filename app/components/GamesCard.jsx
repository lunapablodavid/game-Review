...
import GamesInfo from './GamesInfo'; // Importa el componente GamesInfo si aún no lo has hecho

const GamesCard = ({ imgUrl, title, description, gameUrl, onEyeClick }) => {
        ...
        <div>
            <div className='h-52 md:h-72 rounded-t-xl relative group' style={{ background: `url(${imgUrl})`, backgroundSize: "cover" }}>
                <div className='items-center justify-center absolute top-0 left-0 w-full h-full bg-[#181818] bg-opacity-0 hidden group-hover:flex group-hover:bg-opacity-80 transition-all duration-500'>
                    <button onClick={handleEyeClick} className='h-14 w-14 mr-4 border-2 relative rounded-full border-[#adb7be] hover:border-white group/link'>
                        <EyeIcon className='h-10 w-10 text-[#adb7be] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 cursor-pointer group-hover/link:text-white' />
                    </button>
                    <Link href={gameUrl || '/fallback-url'} className='h-14 w-14 border-2 relative rounded-full border-[#adb7be] hover:border-white group/link'>
